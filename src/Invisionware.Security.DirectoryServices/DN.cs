...
namespace Invisionware.Security.DirectoryServices
{
	/// <summary>
	...
	public class DN
	{
		...
		public DN Parent
		{
			get
			{
				if (RDNs.Any())
				{
					var parentRDNs = RDNs.Skip(1).ToList();
					
					return new DN(new List<RDN>(parentRDNs), this.CharsToEscape);
				}
				else
				{
		...
		public DN(string dnString, EscapeChars escapeChars)
		{
			...
			this.CharsToEscape = escapeChars;
		...
		{
			return ToString(this.CharsToEscape);
		}
		...
		private void ParseDN(string dnString)
		{
			...
			RDNs = rawRDNs.Select(x => new RDN(x)).ToList();
		}
		...
		public DN GetChild(string childRDN)
		{
			...
			if (childDN.RDNs.Count > 0)
			{
				...
				return new DN(new List<RDN>(fullPath), this.CharsToEscape);
			}
			else
			{
				return this;
